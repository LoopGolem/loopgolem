...
The persistence layer also has normalized history tables for task attempts, agent sessions, agent turns and deterministic-recovery episodes. Codex calls now create session/turn records before external execution, observe JSONL incrementally, persist `thread.started` as soon as Codex emits it, and finalize per-turn model, reasoning effort, duration and token usage (including cache-write input tokens when reported). Mission execution policy is persisted independently with defaults of three deterministic-recovery cycles, three validation cycles and session reuse disabled. These records are additive: the existing task-level aggregate fields remain available for compatibility while recovery is introduced incrementally.
