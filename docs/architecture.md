...
The persistence layer also has normalized history tables for the next recovery/session architecture: task attempts, agent sessions, agent turns and deterministic-recovery episodes. Agent-turn records carry model, reasoning effort, per-turn token usage (including cache-write input tokens when Codex reports them), timing and role/purpose metadata. Mission execution policy is persisted independently with defaults of three deterministic-recovery cycles, three validation cycles and session reuse disabled. These records are additive: the existing task-level aggregate fields remain available for compatibility while orchestrator integration is introduced incrementally.
